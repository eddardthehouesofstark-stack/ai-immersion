...
export async function loadDashboardData() {
  ...
  try {
    ...
    if (fastestEl) {
      let fastestObj = null;
      if (stats.fastest_growing && typeof stats.fastest_growing === 'object') {
        fastestObj = stats.fastest_growing;
      } else if (trends.length > 0) {
        fastestObj = [...trends].sort((a, b) => (b.growth_rate || 0) - (a.growth_rate || 0))[0];
      }

      if (fastestObj) {
        const rawName = fastestObj.name || 'Top Mover';
        const truncatedName = String(rawName).slice(0, 22);
        const growthVal = fastestObj.growth_rate ?? (typeof stats.fastest_growing === 'number' ? stats.fastest_growing : 0);
        fastestEl.innerHTML = `+${growthVal}% <span style="font-size: 14px; font-family: var(--font-sans); color: var(--text-secondary); font-weight: normal;">(${truncatedName}...)</span>`;
      } else if (typeof stats.fastest_growing === 'number') {
        fastestEl.innerText = `+${stats.fastest_growing}%`;
      } else {
        fastestEl.innerText = 'N/A';
      }
    }
    if (sourcesEl) sourcesEl.innerText = stats.sources_count || 14;
