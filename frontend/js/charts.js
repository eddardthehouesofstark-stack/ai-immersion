...
export function renderTrendHistoryChart(canvasId, historyPoints, selectedRange = 'all') {
  ...
  const now = Date.now();
  const getTime = (p) => new Date(p?.timestamp || p?.recorded_at || 0).getTime();
  let filtered = [...historyPoints].sort((a, b) => getTime(a) - getTime(b));

  if (selectedRange === '24h') {
    filtered = filtered.filter(p => now - getTime(p) <= 24 * 3600 * 1000);
  } else if (selectedRange === '7d') {
    filtered = filtered.filter(p => now - getTime(p) <= 7 * 24 * 3600 * 1000);
  } else if (selectedRange === '30d') {
    filtered = filtered.filter(p => now - getTime(p) <= 30 * 24 * 3600 * 1000);
  } else if (selectedRange === '90d') {
    filtered = filtered.filter(p => now - getTime(p) <= 90 * 24 * 3600 * 1000);
  }
  ...
  const labels = filtered.map(p => {
    const rawTime = p?.timestamp || p?.recorded_at || Date.now();
